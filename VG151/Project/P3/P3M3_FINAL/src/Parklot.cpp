...
Parklot::Parklot(int _Floor,int _FloorSize):Group(0,0){
	...
	if (_FloorSize==0) _FloorSize=1000; 
	for (int i=0;i<min(5,_FloorSize);i++){
		Line *Line1=new Line(Vec(-0.4+0.2*i,0.7),0.0,0.0,0.0,Vec(0,-0.3));
		Line *Line2=new Line(Vec(-0.4+0.2*i,0.7),0.0,0.0,0.0,Vec(0.2,0));
		Line *Line3=new Line(Vec(-0.4+0.2*(i+1),0.7),0.0,0.0,0.0,Vec(0,-0.3));
		AddNormalFigure({Line1,Line2,Line3});
	}
	_FloorSize-=5;
	for (int i=0;i<min(5,_FloorSize);i++){
		Line *Line1=new Line(Vec(0.6-0.2*i,-0.4),0.0,0.0,0.0,Vec(0,0.3));
		Line *Line2=new Line(Vec(0.6-0.2*i,-0.4),0.0,0.0,0.0,Vec(-0.2,0));
		Line *Line3=new Line(Vec(0.6-0.2*(i+1),-0.4),0.0,0.0,0.0,Vec(0,0.3));
		AddNormalFigure({Line1,Line2,Line3});
	}

	Line *Li=new Line(Vec(-0.8,0.15),0.0,0.0,0.0,Vec(1.2,0));
	AddNormalFigure({Li});
